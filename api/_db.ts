...
export function requireAdmin(req:any,res:any){const expected=process.env.ADMIN_API_TOKEN;if(!expected){res.status(503).json({error:'ADMIN_API_TOKEN is not configured'});return false}const supplied=String(req.headers.authorization||'').replace(/^Bearer\s+/i,'');if(supplied!==expected){res.status(401).json({error:'Unauthorised'});return false}return true}
