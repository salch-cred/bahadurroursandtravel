...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (type === 'migrate') {
      ...
      await sql`CREATE TABLE IF NOT EXISTS invoices (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), invoice_number TEXT UNIQUE NOT NULL, invoice_date TEXT, due_date TEXT, booking_ref TEXT, customer_name TEXT NOT NULL, customer_address TEXT, phone TEXT, email TEXT, items JSONB DEFAULT '[]', tax_label TEXT DEFAULT 'GST', tax_rate NUMERIC DEFAULT 0, discount NUMERIC DEFAULT 0, subtotal NUMERIC DEFAULT 0, tax NUMERIC DEFAULT 0, total NUMERIC DEFAULT 0, status TEXT DEFAULT 'Draft', notes TEXT, payment_details TEXT, pending_amount NUMERIC DEFAULT 0, multi_customers TEXT, kids_names TEXT, payment_remarks TEXT, travel_details JSONB DEFAULT '{}', created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now())`;
    ...
    if (req.method === 'POST' && req.body?.type === 'invoice') {
      ...
      const rows = await sql`insert into invoices(invoice_number, invoice_date, due_date, booking_ref, customer_name, customer_address, phone, email, items, tax_label, tax_rate, discount, subtotal, tax, total, status, notes, payment_details, pending_amount, multi_customers, kids_names, payment_remarks, travel_details) values (${i.invoice_number}, ${i.invoice_date || null}, ${i.due_date || null}, ${i.booking_ref || null}, ${i.customer_name}, ${i.customer_address || null}, ${i.phone || null}, ${i.email || null}, ${JSON.stringify(i.items || [])}::jsonb, ${i.tax_label || 'GST'}, ${i.tax_rate || 0}, ${i.discount || 0}, ${i.subtotal || 0}, ${i.tax || 0}, ${i.total || 0}, ${i.status || 'Draft'}, ${i.notes || null}, ${i.payment_details || null}, ${i.pending_amount || 0}, ${i.multi_customers || null}, ${i.kids_names || null}, ${i.payment_remarks || null}, ${JSON.stringify(i.travel_details || {})}::jsonb) returning *`; // travel_details includes kids, kids_price automatically
    ...
    if (req.method === 'PUT' && req.body?.type === 'invoice') {
      ...
      if (!id) return res.status(400).json({ error: 'id required' });
      const rows = await sql`update invoices set invoice_number=${i.invoice_number}, invoice_date=${i.invoice_date || null}, due_date=${i.due_date || null}, booking_ref=${i.booking_ref || null}, customer_name=${i.customer_name}, customer_address=${i.customer_address || null}, phone=${i.phone || null}, email=${i.email || null}, items=${JSON.stringify(i.items || [])}::jsonb, tax_label=${i.tax_label || 'GST'}, tax_rate=${i.tax_rate || 0}, discount=${i.discount || 0}, subtotal=${i.subtotal || 0}, tax=${i.tax || 0}, total=${i.total || 0}, status=${i.status || 'Draft'}, notes=${i.notes || null}, payment_details=${i.payment_details || null}, pending_amount=${i.pending_amount || 0}, multi_customers=${i.multi_customers || null}, kids_names=${i.kids_names || null}, payment_remarks=${i.payment_remarks || null}, travel_details=${JSON.stringify(i.travel_details || {})}::jsonb where id=${id} returning *`;
      if (!rows.length) return res.status(404).json({ error: 'Invoice not found' });
      return res.status(200).json({ ok: true, invoice: rows[0] });
    }
