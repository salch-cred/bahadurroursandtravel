...
function updateLivePreview(){
  ...
  preview.innerHTML=`
    <div class="live-preview-card">
      <div class="live-preview-img-wrap">
        <img src="${esc(img)}" alt="${esc(name)}" onerror="this.src='assets/images/island-beach.jpg'">
        <span class="live-preview-badge ${active?'live-badge-live':'live-badge-hidden'}">${active?'Live':'Hidden'}</span>
        <span class="live-preview-type">${type==='international'?'International':'Domestic'}</span>
        ${galCount>1?`<span class="live-preview-count"><i class="hgi-stroke hgi-image-01"></i> ${galCount}</span>`:''}
      </div>
      <div class="live-preview-body">
        <div class="live-preview-meta">${[region,duration].filter(Boolean).join(' · ')}</div>
        <h3>${esc(name)}</h3>
        <p>${esc((summary||'').slice(0,90))}${summary.length>90?'…':''}</p>
        <div class="live-preview-price">${esc(price)}</div>
      </div>
    </div>`;
}
